...
class SynonymExpander:
    ...
    MANUAL: dict = {
        # Оргтехника
        "мфу":             ["принтер", "многофункциональное устройство"],
        "принтер":         ["мфу", "многофункциональное устройство"],
        ...
        "флешка":          ["usb накопитель", "флеш накопитель"],
        "клавиатура":      ["клавиатура проводная", "клавиатура беспроводная"],
        "мышь":            ["манипулятор мышь", "мышь компьютерная", "мышка"],
        # Бумага и расходники
        "бумага":          ["бумага офисная", "бумага а4", "бумага а3"],
        "тетрадь":         ["тетрадь школьная", "тетрадь клетка", "тетрадь линейка"],
        "ручка":           ["авторучка", "шариковая ручка", "ручка шариковая"],
        "карандаш":        ["карандаш простой", "карандаш чернографитный"],
        "папка":           ["папка скоросшиватель", "папка файл", "скоросшиватель"],
        "скотч":           ["лента клейкая", "лента скотч"],
        # Мебель
        "стул":            ["стул офисный", "кресло офисное"],
        "кресло":          ["кресло офисное", "стул офисный", "кресло руководителя"],
        "стол":            ["стол офисный", "стол письменный"],
        "шкаф":            ["шкаф офисный", "шкаф для документов", "тумба"],
        "стеллаж":         ["стеллаж металлический", "полка", "полка металлическая"],
        # Медицина и СИЗ
        "перчатки":        ["перчатки медицинские", "перчатки латексные", "перчатки нитриловые"],
        "маска":           ["маска медицинская", "респиратор", "маска защитная"],
        "респиратор":      ["маска медицинская", "маска защитная"],
        "бахилы":          ["бахилы медицинские", "бахилы полиэтиленовые"],
        "антисептик":      ["дезинфектант", "санитайзер", "дезинфицирующее средство"],
        "дезинфектант":    ["антисептик", "санитайзер", "дезинфицирующее средство"],
        "санитайзер":      ["антисептик", "дезинфектант"],
        "мыло":            ["мыло жидкое", "мыло туалетное", "мыло хозяйственное"],
        # Уборка и хозтовары
        "моющее":          ["чистящее средство", "средство для мытья"],
        "мешок":           ["мешок для мусора", "пакет для мусора", "пакет мусорный"],
        "пакет":           ["пакет мусорный", "мешок для мусора"],
        "швабра":          ["швабра с отжимом", "моп", "швабра для мытья"],
        "тряпка":          ["салфетка уборочная", "ветошь", "тряпка уборочная"],
        # Продукты питания
        "чай":             ["чай черный", "чай зеленый", "чай листовой"],
        "кофе":            ["кофе молотый", "кофе растворимый", "кофе в зернах"],
        "сахар":           ["сахар белый", "сахар-рафинад", "сахар-песок"],
        # Стройматериалы
        "доска":           ["доска обрезная", "доска строганая", "пиломатериал"],
        "цемент":          ["цемент м400", "цемент м500", "смесь цементная"],
        "краска":          ["краска водоэмульсионная", "краска акриловая", "лкм"],
        # Медоборудование
        "шприц":           ["шприц одноразовый", "шприц медицинский"],
        "бинт":            ["бинт стерильный", "бинт нестерильный", "перевязочный материал"],
        # IT / Сети
        "кабель":          ["кабель витая пара", "кабель сетевой", "провод"],
    ...
    def expand(self, tokens: list[str]) -> tuple[list[str], list[str]]:
        ...
        for t in tokens:
            ...
            for syn in self._syns.get(t, []):
                syn_tok = syn.split()
                if extra + len(syn_tok) > self.MAX_SYN_EXTRA_TOKENS:
                    continue
                expanded.extend(syn_tok)
                added.append(syn)
                extra += len(syn_tok)
...
class SearchEngine:
    ...
    def load_all(self) -> None:
        ...
        if SYNONYMS_JSON.exists():
            self.synonyms = SynonymExpander(SYNONYMS_JSON)
